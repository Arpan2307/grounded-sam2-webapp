...
apply_grounding_dino_patches() {
    ...
    if grep -q "_C_available = True" "$ms_deform_file"; then
        echo "   ✅ Grounding DINO already patched"
        return
    fi
    
    echo "   Patching Grounding DINO ms_deform_attn.py..."
    
    # Create backup
    if [ ! -f "${ms_deform_file}.original" ]; then
        cp "$ms_deform_file" "${ms_deform_file}.original"
        echo "     Created backup"
    fi
...
if os.path.exists(ms_deform_file):
    ...
    if old_import in content:
        ...
        patched = True
        print('     ✅ Updated _C import to track availability')
    ...
    def forward(
        ...
        if old_forward_start in content:
            ...
            patched = True
            print('     ✅ Updated forward method with _C fallback')
    
    # Patch 3: Update backward method
    backward_section = content.split('def backward(ctx, grad_output):')[1] if 'def backward(ctx, grad_output):' in content else ''
    if 'if not _C_available:' not in backward_section:
    ...
    def backward(ctx, grad_output):
        ...
        if old_backward_start in content:
            content = content.replace(old_backward_start, new_backward_start)
            patched = True
            print('     ✅ Updated backward method with _C fallback')
    ...
    if old_condition in content and new_condition not in content:
        content = content.replace(old_condition, new_condition)
        patched = True
        print('     ✅ Updated CUDA condition to check _C availability')
    
    if patched:
        with open(ms_deform_file, 'w') as f:
            f.write(content)
        print('     ✅ All Grounding DINO patches applied successfully')
    else:
        print('     ℹ️  No Grounding DINO patches needed or already applied')
"
